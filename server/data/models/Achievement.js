...
set(function(v) { this._caption = v; });

AchievementSchema.virtual('star').
get(function() { return this._star; }).
set(function(v) { this._star = v; });

AchievementSchema.virtual('target_point').
get(function() { return this._target_point; }).
set(function(v) { this._target_point = v; });

AchievementSchema.virtual('point').
get(function() { return this._point; }).
set(function(v) { this._point = v; });


AchievementSchema.methods.player = async function(player){
  let achiev = await PlayerAchievement.findOne({achievement: this._id,player: player});
  let detail;
  if(achiev){
    detail = await DetailAchievement.findOne({achievement : this._id, star: (achiev.star+1)});
  } else {
    detail = await DetailAchievement.findOne({achievement : this._id, star: 1});
  }
  if(detail){
